        ...
        Pass
        {
            ...
            v2f vert (float4 vertex : POSITION, float3 normal : NORMAL, float4 tangent : TANGENT, float2 uv : TEXCOORD0, float4 color : COLOR)
            {
                ...
                */
                ...
                /*
                if (dot(viewDir, o.worldNormal) < 0) {
                    o.worldNormal = -o.worldNormal;
                }
                */
            ...
            float4 frag (v2f i) : COLOR
            {
                ...
                */
                ...
                worldNormal = normalize(
                    worldNormal.x * -i.worldTangent + 
                    worldNormal.y * i.worldBitangent + 
                    worldNormal.z * i.worldNormal);
                    */
                worldNormal = normalize(
                    worldNormal.x * i.worldTangent + 
                    worldNormal.y * i.worldBitangent + 
                ...
                //float3 forward = _WorldSpaceCameraPos - mul(UNITY_MATRIX_M, float4(0, 0, 0, 1));
                ...
                /*
                float4 c = float4(
                    (dot(right, worldNormal) * 0.5) + 0.5, 
                    (dot(up, worldNormal) * 0.5) + 0.5, 
                    dot(forward, worldNormal), 1);
                    */
                float4 c = float4(
                    (dot(right, worldNormal) * 0.5) + 0.5,
                    (dot(up, worldNormal) * 0.5) + 0.5, 
