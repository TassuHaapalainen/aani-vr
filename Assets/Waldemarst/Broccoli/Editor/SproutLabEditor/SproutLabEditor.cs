...
namespace Broccoli.BroccoEditor
{
	...
	public class SproutLabEditor {
		...
		public string debugLoadPath = string.Empty;
		#endregion
		...
		/// <summary>
		/// Popup to display snapshot presets.
		/// </summary>
		PopupListComponent snapshotPresetsPopup;
		/// <summary>
		...
		/// </summary>
		private static GUIContent[] panelSectionOption = new GUIContent[5];
		/// <summary>
		...
		public SproutLabEditor (VisualElement parentRootVisualElement) {
			...
			// Popup.
			snapshotPresetsPopup = new PopupListComponent (rootVisualElement);
			InitSnapshotPresetsPopup ();
		...
		private void InitSnapshotPresetsPopup () {
			snapshotPresetsPopup.width = 200;
			snapshotPresetsPopup.height = 130;
			snapshotPresetsPopup.title = "Snapshot Prefabs";
			...
			while (snapEnum.MoveNext ()) {
				...
				snapBtn.clicked += () => {
					...
					snapshotPresetsPopup.Close ();
				};
				snapshotPresetsPopup.listElem.hierarchy.Add (snapBtn);
			}
		}
		#endregion
		...
		public void SetViewLevels (int firstLevelView, int secondLevelView, int thirdLevelView) {
			...
			if (_canvasStructureView == CanvasStructureView.Snapshot) {
				if (currentPanelSection == PANEL_SETTINGS) {
				...
				if (compositeMaterials.Length > 0 && compositeMaterials[0] != null) {
					if (currentMapView == VIEW_COMPOSITE) { // Composite
						currentPreviewMaterials = sproutSubfactory.GetCompositeMaterials (compositeMaterials,
							...
							branchDescriptorCollection.sproutStyleCrown.colorTint,
							...
							SproutSubfactory.GetMaterialCrownStartIndex (branchDescriptorCollection));
						...
						meshPreview.secondPassMaterials = sproutSubfactory.GetAlbedoMaterials (compositeMaterials,
							...
							branchDescriptorCollection.sproutStyleCrown.colorSaturation,
						...
						currentPreviewMaterials = sproutSubfactory.GetExtraMaterials (compositeMaterials,
							...
							branchDescriptorCollection.sproutStyleCrown.metallic,
							branchDescriptorCollection.sproutStyleCrown.glossiness,
							SproutSubfactory.GetMaterialAStartIndex (branchDescriptorCollection),
							SproutSubfactory.GetMaterialBStartIndex (branchDescriptorCollection),
							SproutSubfactory.GetMaterialCrownStartIndex (branchDescriptorCollection));
						...
						currentPreviewMaterials = sproutSubfactory.GetSubsurfaceMaterials (compositeMaterials,
							...
							branchDescriptorCollection.sproutStyleCrown.subsurfaceMul,
							SproutSubfactory.GetMaterialAStartIndex (branchDescriptorCollection),
							SproutSubfactory.GetMaterialBStartIndex (branchDescriptorCollection),
							SproutSubfactory.GetMaterialCrownStartIndex (branchDescriptorCollection));
		...
		public void GenerateNewStructure () {
			...
			if (propagate) {
				if (canvasStructureView == CanvasStructureView.Snapshot) {
					...
					onBeforeEditVariationDescriptor (selectedVariationDescriptor, branchDescriptorCollection);
					sproutSubfactory.ProcessVariation (true);
					onEditVariationDescriptor (selectedVariationDescriptor, branchDescriptorCollection);
					ShowPreviewMesh ();
				}
			}
		...
		/// </summary>
		public void RegenerateStructure (int viewMode = VIEW_COMPOSITE) {
			bool propagate = true;
		...
		public void DrawCompositeMappingSettings () {
			...
			float subsurfaceMulCrown = branchDescriptorCollection.sproutStyleCrown.subsurfaceMul;
			float colorSaturationCrown = branchDescriptorCollection.sproutStyleCrown.colorSaturation;
			...
			if (EditorGUI.EndChangeCheck ()) {
				...
				branchDescriptorCollection.sproutStyleCrown.subsurfaceMul = subsurfaceMulCrown;
				branchDescriptorCollection.sproutStyleCrown.colorSaturation = colorSaturationCrown;
				onEditBranchDescriptor?.Invoke (selectedSnapshot, branchDescriptorCollection);
				ReflectChangesToPipeline ();
				RegenerateStructure (currentMapView);
			}
		}
		#endregion
		...
		void DrawSnapshotsPanel () {
			...
			if (GUILayout.Button (addSnapshotGUI)) {
				if (currentStructureSettings.displaySnapshotPresetsPopup) {
					...
					snapshotPresetsPopup.Open (mousePos);
				} else {
					AddSnapshot ("Main");
				}
			}
		...
		void AddSnapshot (string snapshotType, bool clone = false) {
			if (branchDescriptorCollection.snapshots.Count < 10) {
		...
		/// </summary>
		void AddVariation () {
			if (branchDescriptorCollection.variations.Count < 15) {
		...
		void RemoveVariation () {
			if (EditorUtility.DisplayDialog (MSG_DELETE_VARIATION_DESC_TITLE, 
				...
				branchDescriptorCollection.lastVariationIndex = branchDescriptorCollection.variationIndex;
				TriggerOnBeforeEditVariationDescriptor (selectedVariationDescriptor);
				branchDescriptorCollection.RemoveVariation (selectedVariationDescriptor);
				SelectVariation (0);
				TriggerOnEditVariationDescriptor (selectedVariationDescriptor);
				GUIUtility.ExitGUI ();
			}
		}
		/// <summary>
		...
		public void SelectVariation (int index) {
			...
			if (branchDescriptorCollection.variations.Count > 0 && index >= 0 && index < branchDescriptorCollection.variations.Count) {
				...
				this.selectedVariationDescriptor = branchDescriptorCollection.variations [index];
				this.selectedVariationDescriptor.BuildGroupTree ();
		...
		#region Utils
		public void SetupSlider (Slider slider, bool isInt = false) {
			slider?.RegisterValueChangedCallback(evt => {
				RefreshSlider (slider, evt.newValue, isInt);
			});
		}
		public void SetupMinMaxSlider (MinMaxSlider minMaxSlider, bool isInt = false) {
			minMaxSlider?.RegisterValueChangedCallback(evt => {
				RefreshMinMaxSlider (minMaxSlider, evt.newValue, isInt);
			});
		}
		public void RefreshSlider (Slider slider, float value, bool isInt = false) {
		...
		public void RefreshMinMaxSlider (MinMaxSlider minMaxSlider, Vector2 value, bool isInt = false) {
