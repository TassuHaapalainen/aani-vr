    ...
    public class SproutSubfactory {
        ...
        public void SnapshotCollectionToPipeline () {
            ...
            if (branchBenderElement) {
                branchBenderElement.noiseResolution = snapshotSettings.noiseResolution;
            ...
            if (snapshotSettings.hasCrown && sproutMeshes.Count > 2) {
                ...
                if (sproutMapperElement != null && sproutMapperElement.sproutMaps.Count > 2) {
                    ...
                    sproutMapperElement.sproutMaps [2].subsurfaceValue = 0.5f + Mathf.Lerp (-0.4f, 0.4f, branchDescriptorCollection.sproutStyleCrown.subsurfaceMul - 0.5f);
        ...
        public void ProcessSnapshot (bool clearCompositeManager = true, MaterialMode materialMode = MaterialMode.Composite, bool isNewSeed = false) {
            ...
            if (materialMode == MaterialMode.Albedo) { // Albedo
                ...
                meshRenderer.sharedMaterials = GetExtraMaterials (compositeMaterials,
                    ...
                    GetMaterialAStartIndex (branchDescriptorCollection),
                    GetMaterialBStartIndex (branchDescriptorCollection),
					GetMaterialCrownStartIndex (branchDescriptorCollection));
        ...
        public void ProcessSnapshotPolygons (BranchDescriptor snapshot) {
            ...
            if (sproutCompositeManager.HasSnapshot (snapshot.id)) return;

            // Generate curve.
            GenerateSnapshotCurve (snapshot);
        ...
        public void GenerateSnapshotCurve (BranchDescriptor snapshot) {
        ...
        public void GenerateSnapshotPolygonsPerLOD (int lodLevel, BranchDescriptor snapshot) {
            ...
            if (processor == null) {
                Debug.Log ("No Snapshot Processor found with id " + snapshot + ", skipping processing.");
                return;
            }
            // Begin usage.
        ...
        public void ProcessTextures () {
            ...
            for (int i = 0; i < branchDescriptorCollection.sproutStyleA.sproutMapAreas.Count; i++) {    
                Texture2D texture = ApplyTextureTransformations (
                    ...
                    branchDescriptorCollection.sproutStyleA.sproutMapAlphas [i]);
                if (texture != null) {
            ...
            for (int i = 0; i < branchDescriptorCollection.sproutStyleB.sproutMapAreas.Count; i++) {    
                Texture2D texture = ApplyTextureTransformations (
                    ...
                    branchDescriptorCollection.sproutStyleB.sproutMapAlphas [i]);
                if (texture != null) {
            ...
            for (int i = 0; i < branchDescriptorCollection.sproutStyleCrown.sproutMapAreas.Count; i++) {    
                Texture2D texture = ApplyTextureTransformations (
                    ...
                    branchDescriptorCollection.sproutStyleCrown.sproutMapAlphas [i]);
                if (texture != null) {
                    ...
                    if (i == 0) {
                        ...
                        */
                        ...
                        if (i >= materialAStartIndex) {
                            if (i >= materialCrownStartIndex) {
                                m.SetColor ("_TintColor", tintColorCrown);
                            } else if (i >= materialBStartIndex) {
        ...
        public Material[] GetAlbedoMaterials (Material[] originalMaterials,
            ...
            float materialCrownSaturation = 1f,
            ...
            for (int i = 0; i < originalMaterials.Length; i++) {
                ...
                if (originalMaterials [i] == null) {
                    ...
                    if (i >= materialAStartIndex) {
                        if (i >= materialCrownStartIndex) {
                            m.SetColor ("_TintColor", tintColorCrown);
                            m.SetFloat ("_SproutSat", materialCrownSaturation);
                        } else if (i >= materialBStartIndex) {
                            m.SetColor ("_TintColor", tintColorB);
        ...
        { 
            ...
            for (int i = 0; i < originalMaterials.Length; i++) {
                ...
                if (i >= materialAStartIndex) {
                    if (i >= materialCrownStartIndex) {
                        m.SetFloat ("_Metallic", metallicCrown);
                        m.SetFloat ("_Glossiness", glossinessCrown);
                    } else if (i >= materialBStartIndex) {
        ...
        public Material[] GetSubsurfaceMaterials (Material[] originalMaterials,
            ...
            float sproutCrownSaturation = 1f,
            float sproutASubsurfaceMul = 1f,
            float sproutBSubsurfaceMul = 1f,
            float sproutCrownSubsurfaceMul = 1f,
            int materialAStartIndex = -1,
            int materialBStartIndex = -1,
            int materialCrownStartIndex = -1) 
        {
            ...
            for (int i = 0; i < originalMaterials.Length; i++) {
                ...
                if (i >= materialAStartIndex) {
                    if (i >= materialCrownStartIndex) {
                        m.SetColor ("_TintColor", tintColorCrown);
                        m.SetFloat ("_SproutSat", sproutCrownSaturation);
                        m.SetFloat ("_SproutBr", sproutCrownSubsurfaceMul);
                    } else if (i >= materialBStartIndex) {
                        m.SetColor ("_TintColor", tintColorB);
