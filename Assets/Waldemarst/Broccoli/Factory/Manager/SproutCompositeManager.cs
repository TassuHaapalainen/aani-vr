...
namespace Broccoli.Manager
{
	...
	public class SproutCompositeManager {
		...
		public bool UnmanagePolygonAreas (int snapshotId) {
			if (!_idToSnapshot.ContainsKey (snapshotId)) {
				BranchDescriptor snapshot = _idToSnapshot [snapshotId];
				if (snapshot != null) {
					...
					_idToSnapshot.Remove (snapshotId);
					return true;
				}
			}
			return false;
		...
		public Mesh GetMesh (int snapshotId, int lod, int resolution = 0, bool useCache = true) {
			...
			int branchLOD = snapshotId * 1000 + lod * 100 + resolution;
			if (_snapshotIdToMesh.ContainsKey (branchLOD) && useCache) {
				return _snapshotIdToMesh [branchLOD];
			} else {
				...
				if (_snapshotIdToMesh.ContainsKey (branchLOD)) {
					UnityEngine.Object.DestroyImmediate (_snapshotIdToMesh [branchLOD]);
					_snapshotIdToMesh.Remove (branchLOD);
				}
				_snapshotIdToMesh.Add (branchLOD, mesh);
				return mesh;
			}
		}
		/// <summary>
		...
		public bool GenerateTextures (PolygonArea polygonArea, BranchDescriptor snapshot, SnapshotProcessor.Fragment fragment, SproutSubfactory sproutFactory) {
			...
			if (_snapshotTree != null/*_snapshotMesh != null*/) {
				...
				sproutFactory.GeneratePolygonTexture (_snapshotMesh, polygonArea.aabb, 
